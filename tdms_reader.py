...
# -------------------- Lettura completa in una sola apertura --------------------

def read_all_data(tdms_path: str, test_index: int = 0,
                  on_progress=None) -> dict:
    """
    Apre il file TDMS una sola volta e legge tutti i dati necessari:
      - contract:        dati contrattuali e loop (read_contract_and_loop_data)
      - perf:            tabelle performance (read_performance_tables_dynamic)
      - power_calc_type: tipo calcolo potenza (read_power_calc_type)

    Usato da _build_cache in certificate_logic per evitare aperture multiple.
    """
    result = {
        "contract":        {},
    ...
    try:
        _prog(10, "Opening TDMS file...")
        # TdmsFile() = carica tutto in RAM in una passata sequenziale
        # su rete e' piu' veloce di TdmsFile.open() che fa seek random
    ...
    try:
        # ── Contract data ─────────────────────────────────────────────────────
        out = {
            "Capacity [m3/h]": "", "TDH [m]": "", "Efficiency [%]": "",
            "ABS_Power [kW]": "", "Speed [rpm]": "", "SG Contract": "",
            "Temperature [°C]": "", "Viscosity [cP]": "", "NPSH [m]": "",
            "Liquid": "", "FSG ORDER": "", "Customer": "",
            "Purchaser Order": "", "End User": "", "Applic. Specs.": "",
            "Item": "", "Pump": "", "Serial Number_Elenco": "",
            "Impeller Drawing": "", "Impeller Material": "", "Diam Nominal": "",
            "Suction [Inch]": "", "Discharge [Inch]": "", "Wattmeter Const.": "",
            "AtmPress [m]": "", "KNPSH [m]": "", "WaterTemp [°C]": "",
            "KVenturi": "",
        }
        out["Capacity [m3/h]"] = _read_scalar_from_tdms(tdms, "Ref. Contract Data", "Capacity [m3/h]")
        out["TDH [m]"]         = _read_scalar_from_tdms(tdms, "Ref. Contract Data", "TDH [m]")
        ...
        # ── Performance tables ────────────────────────────────────────────────
        _prog(30, "Reading performance tables...")
        ...
        if points:
            ...
            custom_labels = []
            if test_index == 0:
                # Leggi Perfor_Table_Label da Info_Table (gia' con tdms aperto)
                try:
                    info_group = _get_group_ci(tdms, "Info_Table")
                    if info_group:
                        label_ch = _get_channel_ci(info_group, "Perfor_Table_Label")
                        if label_ch:
                            try:
                                label_data = label_ch[:]
                            except Exception:
                                label_data = getattr(label_ch, "data", [])
                            for raw_label in label_data:
                                ...
                                if len(parts) >= 2:
                                    ...
                                    if "[" in name and "]" in name:
                                        ...
                                        custom_labels.append(f"{name} [{unit}]")
                                    else:
                                        custom_labels.append(name)
                                elif len(parts) == 1 and parts[0].strip():
                                    custom_labels.append(parts[0].strip())
                except Exception:
                    pass

            perf_out = {"Recorded": {"columns": [], "rows": []},
                        "Calc":     {"columns": [], "rows": []},
                        "Converted":{"columns": [], "rows": []}}
            for k in ("Recorded", "Calc", "Converted"):
                if by_kind[k]:
                    _kind_pct = {"Recorded": 40, "Calc": 50, "Converted": 55}
                    _prog(_kind_pct.get(k, 50), f"Reading {k} data...")
                    cols, rows = _build_kind_model(by_kind[k])
                    if k == "Recorded" and custom_labels and test_index == 0:
                        num_cols   = len(cols)
                        num_labels = len(custom_labels)
                        cols = custom_labels[:num_cols] if num_labels >= num_cols                                else custom_labels + cols[num_labels:]
                    perf_out[k]["columns"] = cols
                    perf_out[k]["rows"]    = rows
            result["perf"] = perf_out

        # ── Power calc type ───────────────────────────────────────────────────
        try:
            info_group = _get_group_ci(tdms, "Info_Table")
            if info_group:
                val_ch  = _get_channel_ci(info_group, "Power_Calc_Type_Value")
                list_ch = _get_channel_ci(info_group, "Power_Calc_Type_Elenco")
                if val_ch and list_ch:
                    try:
                        val_data = val_ch[:]
                    except Exception:
                        val_data = getattr(val_ch, "data", [])
                    try:
                        list_data = list_ch[:]
                    except Exception:
                        list_data = getattr(list_ch, "data", [])
                    idx_str = _first_nonempty(val_data)
                    if idx_str:
                        try:
                            idx = int(float(str(idx_str).replace(",", ".").strip()))
                            elenco = [str(x).strip() for x in list_data if _first_nonempty([x])]
                            if 0 <= idx < len(elenco):
                                result["power_calc_type"] = elenco[idx] or "-"
                        except Exception:
                            pass
        except Exception:
            pass
...
def _normalize_channel_name(ch_name: str) -> str:
    ...
    Rimuove anche % duplicati nel nome (es. "TRANSM. EFF % %" -> "TRANSM. EFF %").
    """
    name = (ch_name or "").strip()
    if not name:
        return "—"
    name = re.sub(r"\s+", " ", name).strip()
    # Rimuovi % duplicati (es. "EFF % %" -> "EFF %")
    name = re.sub(r"(%\s*)+", "%", name).strip()
    # Rimuovi % finale se e' gia' presente come unita' tra []
    name = re.sub(r"\[%\]\s*%", "[%]", name).strip()
    # Converti % finale standalone in [%] per coerenza
    # Es. "TRANSM. EFF %" -> "TRANSM. EFF [%]"
    name = re.sub(r"\s+%$", " [%]", name).strip()
    return name
...
def detect_test_types(tdms_path: str) -> list:
    ...
    try:
        # read_metadata: legge solo la struttura, non i dati (veloce su rete)
