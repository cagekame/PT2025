...
def _ensure_tabelle_collaudi(conn: sqlite3.Connection) -> None:
    ...
    # Tabella note: chiave acquisizione_id (coerente con curve_settings)
    cur.execute("""
...
def _ensure_hidden_rows_table(conn: sqlite3.Connection) -> None:
    """Garantisce l'esistenza della tabella righe nascoste."""
    cur = conn.cursor()
    cur.execute("""
...
def init() -> None:
    ...
    with connect() as conn:
        ...
        _ensure_hidden_rows_table(conn)
        _ensure_loop_detail_columns(conn)
        conn.commit()
...
def _ensure_loop_detail_columns(conn=None):
    """Aggiunge le colonne test_cell e hydraulic se non esistono."""
    def _do(c):
        for col in ("test_cell", "hydraulic"):
            try:
                cursor = c.execute("PRAGMA table_info(acquisizioni)")
                cols = [row[1] for row in cursor.fetchall()]
                if col not in cols:
                    c.execute(f"ALTER TABLE acquisizioni ADD COLUMN {col} TEXT DEFAULT ''")
            except Exception:
                pass
    if conn is not None:
        _do(conn)
...
def get_loop_detail(acq_id: int) -> dict:
    """Restituisce test_cell e hydraulic per acquisizione."""
    if not acq_id:
        return {"test_cell": "", "hydraulic": ""}
    try:
        with connect() as conn:
            row = conn.execute(
                "SELECT test_cell, hydraulic FROM acquisizioni WHERE id=?", (acq_id,)
            ).fetchone()
            if row:
                return {"test_cell": row[0] or "", "hydraulic": row[1] or ""}
    except Exception:
        pass
    return {"test_cell": "", "hydraulic": ""}


def set_loop_detail(acq_id: int, test_cell: str, hydraulic: str) -> None:
    """Salva test_cell e hydraulic per acquisizione."""
    if not acq_id:
        return
    try:
        with connect() as conn:
            conn.execute(
                "UPDATE acquisizioni SET test_cell=?, hydraulic=? WHERE id=?",
                (test_cell or "", hydraulic or "", acq_id)
            )
            conn.commit()
    except Exception:
        pass
...
def hidden_rows_get(acq_id: Optional[int], test_type: str) -> set:
    """Restituisce il set di iid nascosti per acquisizione e test_type."""
    if acq_id is None:
        return set()
    try:
        with connect() as conn:
            _ensure_hidden_rows_table(conn)
            rows = conn.execute(
                "SELECT row_iid FROM hidden_rows "
                "WHERE acquisizione_id=? AND test_type=?",
                (acq_id, test_type)
            ).fetchall()
...
def hidden_rows_set(acq_id: Optional[int], test_type: str,
                    hidden: set) -> None:
    """Salva il set di iid nascosti per acquisizione e test_type."""
    if acq_id is None:
        return
    try:
        with connect() as conn:
            ...
            for iid in hidden:
                conn.execute(
                    "INSERT OR IGNORE INTO hidden_rows "
                    "(acquisizione_id, test_type, row_iid) VALUES(?,?,?)",
                    (acq_id, test_type, iid)
                )
            conn.commit()
    except Exception:
        pass
...
def delete_acquisizione(acq_id: int) -> None:
    """
    Cancella il record dalla tabella acquisizioni.
    Le note collegate vengono rimosse automaticamente per ON DELETE CASCADE.
    """
    with connect() as conn:
...
def note_collaudatore_get(acq_id: int) -> str:
    """Ritorna la nota del collaudatore per l'acquisizione indicata."""
    if acq_id is None:
        return ""
    with connect() as conn:
        row = conn.execute(
            "SELECT note_collaudatore FROM notes WHERE acquisizione_id = ?",
            (acq_id,)
        ).fetchone()
        return row[0] if row and row[0] else ""


def note_collaudatore_set(acq_id: int, note: str) -> None:
    """Imposta la nota del collaudatore per l'acquisizione indicata."""
    if acq_id is None:
        return
    with connect() as conn:
        conn.execute("""
            INSERT INTO notes(acquisizione_id, note_collaudatore)
            VALUES(?, ?)
            ON CONFLICT(acquisizione_id) DO UPDATE SET
...
def note_ingegneria_get(acq_id: int) -> str:
    """Ritorna la nota di ingegneria per l'acquisizione indicata."""
    if acq_id is None:
        return ""
    with connect() as conn:
        row = conn.execute(
            "SELECT note_ingegneria FROM notes WHERE acquisizione_id = ?",
            (acq_id,)
        ).fetchone()
        return row[0] if row and row[0] else ""


def note_ingegneria_set(acq_id: int, note: str) -> None:
    """Imposta la nota di ingegneria per l'acquisizione indicata."""
    if acq_id is None:
        return
    with connect() as conn:
        conn.execute("""
            INSERT INTO notes(acquisizione_id, note_ingegneria)
            VALUES(?, ?)
            ON CONFLICT(acquisizione_id) DO UPDATE SET
