...
def _split_header_and_unit_simple(ch_name: str):
...
def _fmt_if_number(value):
    """
    Se `value` è numerico, applica _fmt_num (3 decimali, half-up, senza zeri finali).
    Altrimenti restituisce il valore originale (o '—' se vuoto).
    ...
    if s == "" or s == "—":
        return "—"
    try:
        # supporta "," come decimale
        s_num = s.replace(",", ".")
        Decimal(s_num)  # solo test
...
# ===== Builder semplice: colonne dai nomi canale (primo token + primo qualificatore) =====
def _build_kind_model_no_unit(groups_by_point: dict):
    ...
    def base_key_no_unit(ch_name: str):
        header, unit = _split_header_and_unit_simple(ch_name)
        if not header or header == "—":
            return None, None
        return header, unit

    # Scansione per definire ordine colonne, unità preferita e duplicati massimi
        ...
        for grp in groups_by_point[p]:
            for ch in grp.channels():
                ...
                if key not in preferred_unit:
                    preferred_unit[key] = unit  # qui resta ""
    ...
    # Costruzione intestazioni finali (con suffissi __2, __3... quando servono)
    columns, units = [], []
    for key in first_seen_order:
        ...
        for i in range(1, n + 1):
            ...
            columns.append(col)
            units.append(preferred_unit.get(key, ""))  # vuoto
...
def open_detail_window(root, columns, values, meta):
    ...
    # Stato iniziale: TDMS dalla riga selezionata (dashboard)
    ...
    curva_tab = tk.Frame(nb, bg="#f0f0f0")
    nb.add(curva_tab, text="Curva")
    ...
    # (RIMOSSO) Pulsante "Carica TDMS…"
    lbl_cert = tk.Label(right, text=f"N° Cert.: {cert_num}", bg="#f0f0f0", font=("Segoe UI", 10)); lbl_cert.pack(anchor="e")
    tk.Label(right, text="U.M. System: SI (Metric)", bg="#f0f0f0", font=("Segoe UI", 10)).pack(anchor="e")
    lbl_date = tk.Label(right, text=f"Test Date: {test_date}", bg="#f0f0f0", font=("Segoe UI", 10)); lbl_date.pack(anchor="e")
    ...
    def render_contract_and_loop(tdms_path: str):
        ...
        if NPTDMS_OK and tdms_path and os.path.exists(tdms_path):
            try:
                ...
                # supporta sia Temperature [°C] sia [C] (se presenti nel TDMS)
                temp    = tdms_read_scalar_string(tdms, "Ref. Contract Data", "Temperature [°C]") or \
                          tdms_read_scalar_string(tdms, "Ref. Contract Data", "Temperature [C]")
                ...
                watertemp   = tdms_read_scalar_string(tdms, "Ref. Test Detail", "WaterTemp [°C]") or \
                              tdms_read_scalar_string(tdms, "Ref. Test Detail", "WaterTemp [C]")
        ...
        # ---- formatting numerico condizionato ----
        cap     = _fmt_if_number(cap)
        tdh     = _fmt_if_number(tdh)
        eff     = _fmt_if_number(eff)
        abs_pow = _fmt_if_number(abs_pow)
        speed   = _fmt_if_number(speed)
        sg      = _fmt_if_number(sg)
        temp    = _fmt_if_number(temp)
        visc    = _fmt_if_number(visc)
        npsh    = _fmt_if_number(npsh)
        # liquid può essere testo, lo lasciamo invariato

        suction    = _fmt_if_number(suction)
        discharge  = _fmt_if_number(discharge)
        watt_const = _fmt_if_number(watt_const)
        atmpress   = _fmt_if_number(atmpress)
        knpsh      = _fmt_if_number(knpsh)
        watertemp  = _fmt_if_number(watertemp)
        kventuri   = _fmt_if_number(kventuri)
    ...
    def render_tables(tdms_path: str):
        ...
        def _apply_minsize_once():
            ...
            outer_tables = _pack_padx_total(tables_row)  # 32
            outer_nb     = _pack_padx_total(nb)          # 20
    ...
    # Render iniziale: usa direttamente il TDMS passato dalla dashboard (se esiste)
