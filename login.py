...
import dashboard_logic as dashboard
import db
...
def cambia_db():
    ...
    # Apre il DB esistente e aggiorna lo schema se necessario
    if not os.path.exists(path):
        messagebox.showerror(
            "Database not found",
            "The selected file does not exist.\n\n"
            "Please select an existing database."
        )
        return
    db.ensure_full_schema(path, create_if_missing=False)
...
def login():
    ...
    if not db.db_file_exists(db_path):
        messagebox.showerror(
            ...
            "Please connect an existing database using the 'Change DB' button."
        )
        return
...
def chiedi_password_admin(on_success):
    """Apre dialog verifica password admin. Chiama on_success() se corretta."""
...
btn_crea_utente = tk.Button(
    root,
    text="Crea nuovo utente",
    command=lambda: chiedi_password_admin(apri_finestra_crea_utente),
    width=20,
    font=("Helvetica", 10, "bold")
)
...
btn_elimina_utente = tk.Button(
    root,
    text="Elimina utente",
    command=lambda: chiedi_password_admin(apri_finestra_elimina_utente),
    width=20,
    font=("Helvetica", 10, "bold")
)
...
if not db.db_file_exists(db_path):
    messagebox.showwarning(
        ...
        "No working database found.\n\n"
        "Please connect an existing database using the 'Change DB' button.\n\n"
        "If you need to create a new database, contact your administrator."
    )
    label_db.config(text="No database connected")
