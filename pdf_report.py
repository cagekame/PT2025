...
from tdms_reader import (
    ...
    read_tdms_fields,
    read_power_calc_type,
)

from ui_format import fmt_if_number, fmt_seq, clean_header_brackets, normalize_headers, DASH
...
def generate_pdf_report_like_standard(
    *,
    ...
    # ── Determina test_index dal tipo_test ───────────────────────────────────
    _TEST_INDEX_MAP = {"PERFORMANCE": 0, "NPSH": 1, "RUNNING": 2}
    test_index = _TEST_INDEX_MAP.get((tipo_test or "").upper(), 0)

    # ── Leggi dati da ptcache se disponibile, altrimenti da TDMS ────────────
    _cache = None
    if tdms_path:
        try:
            import ptcache as _ptcache
            if _ptcache.exists(tdms_path):
                _cache = _ptcache.load(tdms_path, test_index=test_index,
                                       unit_system=unit_system)
        except Exception:
            pass

    if _cache:
        contract = _cache.get("contract", {})
        perf     = _cache.get("perf", {})
    else:
        # Fallback: leggi dal TDMS
        contract = read_contract_and_loop_data(tdms_path) if tdms_path else {}
        if unit_system != "Metric":
            try:
                import unit_converter as uc
                contract = uc.convert_contractual_data(contract, "Metric", unit_system)
            except Exception:
                pass
        perf = read_performance_tables_dynamic(tdms_path, test_index=test_index)                if tdms_path else {"Recorded": {}, "Calc": {}, "Converted": {}}
        if unit_system != "Metric":
            try:
                import unit_converter as uc
    ...
    def get_contract_value(key: str) -> str:
        """
        Legge una chiave dal contract con fallback robusto.
        Cerca per nome base (ignorando le unita') se la chiave esatta non esiste.
        """
        v = contract.get(key, "")
        if v:
            return str(v)
        # Prova varianti ASCII/Unicode
        alts = []
        if "m³/h" in key:
            alts.append(key.replace("m³/h", "m3/h"))
        elif "m3/h" in key:
            alts.append(key.replace("m3/h", "m³/h"))
        if "°C" in key:
            alts.append(key.replace("°C", "C"))
        elif "[C]" in key:
            alts.append(key.replace("[C]", "[°C]"))
        if "°F" in key:
            alts.append(key.replace("°F", "F"))
        for alt in alts:
            v = contract.get(alt, "")
            if v:
                return str(v)
        # Fallback finale: cerca per nome base ignorando le unita'
        import re as _re
        base = _re.sub(r"\s*\[[^\]]*\]\s*$", "", key).strip().lower()
        for k, val in contract.items():
            k_base = _re.sub(r"\s*\[[^\]]*\]\s*$", "", k).strip().lower()
            if k_base == base and val:
                return str(val)
        return ""

    # Numero certificato — dalla cache se disponibile, altrimenti TDMS
    if _cache:
        cert_num = n_collaudo or DASH
    elif tdms_path:
        tdms_fields = read_tdms_fields(tdms_path)
    ...
    contractual_kv = [
        (f"Capacity [{flow_unit}]",    get_contract_value(f"Capacity [{flow_unit}]")),
        (f"TDH [{head_unit}]",         get_contract_value(f"TDH [{head_unit}]")),
        ("Efficiency [%]",             get_contract_value("Efficiency [%]")),
        (f"ABS_Power [{power_unit}]",  get_contract_value(f"ABS_Power [{power_unit}]")),
        ("Speed [rpm]",                get_contract_value("Speed [rpm]")),
        ("SG",                         get_contract_value("SG Contract")),
        (f"Temperature [{temp_unit}]", get_contract_value(f"Temperature [{temp_unit}]")),
        ("Viscosity [cP]",             get_contract_value("Viscosity [cP]")),
        (f"NPSH [{npsh_unit}]",        get_contract_value(f"NPSH [{npsh_unit}]")),
    ...
    # Leggi tipo calcolo potenza — dalla cache se disponibile
    power_calc_type = (_cache.get("power_calc_type") if _cache else None) \
                      or (read_power_calc_type(tdms_path) if tdms_path else "") \
                      or ""

    # Leggi test_cell e hydraulic dal DB
    _loop_detail_pdf = {"test_cell": "", "hydraulic": ""}
    try:
        from db import get_loop_detail as _gld_pdf
        _loop_detail_pdf = _gld_pdf(acquisizione_id) if acquisizione_id else _loop_detail_pdf
    except Exception:
        pass

    loop_kv = [
        ("Test performed with :", power_calc_type or ""),
        ...
        (f"AtmPress [{head_unit}]", contract.get(f"AtmPress [{head_unit}]", contract.get("AtmPress [m]", ""))),
        (f"KNPSH [{npsh_unit}]",    contract.get(f"KNPSH [{npsh_unit}]",    contract.get("KNPSH [m]", ""))),
        (f"WaterTemp [{temp_unit}]", contract.get(f"WaterTemp [{temp_unit}]", contract.get("WaterTemp [°C]", contract.get("WaterTemp [C]", "")))),
        ("Kventuri",              contract.get("KVenturi", "")),
        ("Test Cell",             _loop_detail_pdf.get("test_cell", "") or ""),
        ("Hydraulic",             _loop_detail_pdf.get("hydraulic", "") or ""),
    ]
    ...
    # Se i dati vengono dalla cache, le colonne potrebbero non avere unita'
    # Aggiungiamo le unita' tramite unit_converter
    if _cache:
        try:
            import unit_converter as _uc
            from unit_converter import normalize_recorded_columns
            rec_cols = normalize_recorded_columns(
                rec_cols, unit_system=unit_system,
                has_units=(test_index == 0))
            cal_cols, cal_rows = _uc.convert_performance_table(
                cal_cols, cal_rows, unit_system, unit_system)
            con_cols, con_rows = _uc.convert_performance_table(
                con_cols, con_rows, unit_system, unit_system)
        except Exception:
            pass
    ...
    # Escape % per ReportLab (% è carattere speciale nei Paragraph)
    def _esc(s):
        return str(s).replace("%", "&#37;") if s else ""

    hdr_row_names = [Paragraph(f"<b>{_esc(_safe(x))}</b>", P["tiny_center"]) for x in col_names]
    hdr_row_units = [Paragraph(_esc(_safe(u)), P["tiny_center"]) for u in col_units]
    ...
    _hidden_rows_pdf = set()
    try:
        from db import hidden_rows_get as _hrg_pdf
        _test_type_pdf = {0: "PERFORMANCE", 1: "NPSH", 2: "RUNNING"}.get(test_index, "PERFORMANCE")
        _hidden_rows_pdf = _hrg_pdf(acquisizione_id, _test_type_pdf) if acquisizione_id else set()
    except Exception:
        pass

    if tdms_path or _cache:
        try:
            from reportlab.platypus import PageBreak
            ...
            if tipo_test.upper() == "NPSH":
                # Curva NPSH
                from npsh_logic import build_npsh_figure
                npsh_fig, _ = build_npsh_figure(
                    tdms_path,
                    show_points=cs["show_points"],
                    unit_system=unit_system,
                    cache=_cache,
                    hidden_rows=_hidden_rows_pdf,
                )
                add_curve_page(npsh_fig, "NPSH Curve")
            else:
                # Curve PERFORMANCE (default)
                from curve_logic import build_tdh_eff_figure, build_power_figure
...
def preview_pdf_report(
    ...
    *,
    ...
    try:
        ...
        note_coll = note_collaudatore_get(acquisizione_id) or ""
        note_ing  = note_ingegneria_get(acquisizione_id) or ""

        _prog(40, "Building PDF...")
